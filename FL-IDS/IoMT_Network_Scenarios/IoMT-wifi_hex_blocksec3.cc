...
int main(int argc, char *argv[]) 
{
    ...
    std::string attackType    = "none";               // “none” or “attack”
    uint32_t    seedValue     = 1;                    // RNG seed
    std::string flowOutputPath = "flowmonitor-stats_hex_blocksec_normal.xml";

    CommandLine cmd;
    cmd.AddValue("attackType", "Type of attack: none / attack", attackType);
   ...
   p2p.SetDeviceAttribute("DataRate", StringValue("3Mbps")); // Nominal BLE speed
   p2p.SetChannelAttribute("Delay", StringValue("2ms"));
   
   // Add random jitter to delay (0–5 ms extra)
   Ptr<UniformRandomVariable> jitter = CreateObject<UniformRandomVariable>();
   jitter->SetAttribute("Min", DoubleValue(0.0));
   jitter->SetAttribute("Max", DoubleValue(0.005)); // 5 ms max extra delay

   double jitterSeconds = jitter->GetValue(); // sample once
   Time delay = Seconds(0.002 + jitterSeconds); // base 2ms + sampled jitter
   ...
   // Apply packet loss to simulate interference
   Ptr<RateErrorModel> lossModel = CreateObject<RateErrorModel>();
   lossModel->SetAttribute("ErrorRate", DoubleValue(0.02)); // 2% packet loss typical for BLE noise
   ...
   // Bluetooth traffic app
   OnOffHelper bluetoothTraffic("ns3::UdpSocketFactory", bluetoothSinkAddress);
   bluetoothTraffic.SetAttribute("DataRate", StringValue("2Kbps")); // Very low BLE telemetry rate
   bluetoothTraffic.SetAttribute("PacketSize", UintegerValue(50));  // Small BLE-style packets
   ...
   // OPTIONAL: simulate occasional connection drop every ~20s
   Simulator::Schedule(Seconds(20.0), &NetDevice::SetMtu, p2pDevices.Get(0), 0); // Drop link
   Simulator::Schedule(Seconds(21.0), &NetDevice::SetMtu, p2pDevices.Get(0), 1500); // Reconnect
   Simulator::Schedule(Seconds(40.0), &NetDevice::SetMtu, p2pDevices.Get(0), 0);
   Simulator::Schedule(Seconds(41.0), &NetDevice::SetMtu, p2pDevices.Get(0), 1500);
    ...
    // Enable PCAP tracing
    phy.EnablePcap("wifi-ap_hex_blocksec", apDevice);
    phy.EnablePcap("baxter_hex_blocksec", staDevices.Get(0));
    p2p.EnablePcap("hexoskin_hex_blocksec", p2pDevices);
    phy.EnablePcap("hexoskin_phone_hex_blocksec", staDevices.Get(1));
    phy.EnablePcap("hexoskin_phone_hex_blocksec", staDevices.Get(2));

    // NetAnim
    AnimationInterface anim("network-anim_hex_blocksec.xml");
