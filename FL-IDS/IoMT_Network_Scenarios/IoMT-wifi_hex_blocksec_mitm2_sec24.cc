...
public:
    ComprehensiveAssessmentLogger(const std::string& filename) {
        ...
        csvFile
            ...
            // FL-IDS (real local-model + FedAvg outputs)
            << "FL_LocalProb,FL_LocalLoss,FL_Round,"
            // Dominant metric and mitigation
            << "DominantMetric,Mitigation\n";
    }

    void LogAssessment(const ComprehensiveRiskAssessment& a,
                        double flProb = 0.0, double flLoss = 0.0, int flRound = 0) {
        ...
        csvFile << std::fixed << std::setprecision(6)
            ...
            // FL-IDS real model outputs
            << flProb << "," << flLoss << "," << flRound << ","
            // summary
            << a.dominantMetric << ","
            << "\"" << a.mitigation << "\"\n";
        csvFile.flush();
...
// ================================================================================
// ========== REAL FEDERATED LEARNING CORE: Local Models + Multi-Round FedAvg ====
// ================================================================================
// Replaces the previous "scalar aggregation" placeholder (rolling mean of loss
// rates) with an actual federated-learning pipeline:
//   1. Each node keeps a LOCAL MODEL (logistic-regression-style single-layer
//      perceptron) over the standardised (TRR, PLR, NDI, JVI) feature vector.
//   2. Each node accumulates a LOCAL TRAINING SET of (features, label) samples
//      as flow statistics arrive. The label is the ground-truth attack state of
//      the simulation (mitmActive) at the time the sample was taken — this is
//      the supervisory signal a real deployment would substitute with confirmed
//      incident reports / clinician-verified alerts.
//   3. Each communication round, every node runs a LOCAL TRAINING LOOP (several
//      epochs of mini-batch gradient descent) on its buffered samples.
//   4. The trained per-node weight vectors are aggregated with WEIGHTED FEDAVG
//      (weighted by each node's local sample count) into a new global model.
//   5. The global weights are broadcast back down to every node before the next
//      round, exactly as in standard synchronous FedAvg.
// ================================================================================

#include <array>
#include <random>

struct FeatureLabelSample {
    std::array<double, 4> features; // {TRR, PLR, NDI, JVI}, each pre-scaled to [0,1]
    double label;                   // 1.0 = attack (ground truth), 0.0 = normal
};
...
public:
    ...
    explicit LocalModel(double lr = 0.15) : bias(0.0), learningRate(lr) {
        // Small, deterministic-per-run random init (broken symmetry, reproducible with RngSeed)
    ...
    double Forward(const std::array<double, NUM_FEATURES>& x) const {
        double z = bias;
        for (size_t i = 0; i < NUM_FEATURES; ++i) z += weights[i] * x[i];
        return Sigmoid(z);
    }

    // One epoch of full-batch gradient descent (binary cross-entropy loss) over
    // the supplied local training set. Returns the mean loss for this epoch.
    double TrainEpoch(const std::vector<FeatureLabelSample>& batch) {
        if (batch.empty()) return 0.0;

        std::array<double, NUM_FEATURES> gradW{};
        gradW.fill(0.0);
        double gradB = 0.0;
        ...
        for (const auto& s : batch) {
            double pred = Forward(s.features);
            double err  = pred - s.label;              // dL/dz for sigmoid + BCE
            for (size_t i = 0; i < NUM_FEATURES; ++i)
                gradW[i] += err * s.features[i];
            gradB += err;

            double p = std::min(std::max(pred, eps), 1.0 - eps);
            totalLoss += -(s.label * std::log(p) + (1.0 - s.label) * std::log(1.0 - p));
        }
    ...
    // Real local training loop: multiple epochs of gradient descent per FL round.
    double LocalTrainingLoop(const std::vector<FeatureLabelSample>& data, int epochs) {
        double lastLoss = 0.0;
    ...
    double Accuracy(const std::vector<FeatureLabelSample>& data) const {
        if (data.empty()) return 0.0;
        size_t correct = 0;
        for (const auto& s : data) {
            int pred = Forward(s.features) >= 0.5 ? 1 : 0;
            if (pred == static_cast<int>(s.label)) correct++;
        }
        return static_cast<double>(correct) / data.size();
    }

    // Serialise weights (+bias) for transmission to the aggregator.
    ...
    // Load weights (+bias) received from the aggregator (global model broadcast).
    void SetParameters(const std::vector<double>& p) {
        for (size_t i = 0; i < NUM_FEATURES && i < p.size(); ++i)
            weights[i] = p[i];
        if (p.size() > NUM_FEATURES) bias = p[NUM_FEATURES];
    }
};

// ---- Weighted FedAvg: aggregate per-client parameter vectors into a global one ----
// Weight of client c = (client c's local dataset size) / (sum of all dataset sizes),
// exactly as in McMahan et al.'s FedAvg algorithm.
std::vector<double> FedAvgAggregate(const std::vector<std::vector<double>>& clientParams,
                                     const std::vector<size_t>& clientDataSizes) {
    if (clientParams.empty()) return {};
    size_t paramLen = clientParams[0].size();
    std::vector<double> avg(paramLen, 0.0);

    size_t totalSamples = std::accumulate(clientDataSizes.begin(), clientDataSizes.end(), (size_t)0);
    if (totalSamples == 0) {
        // No client has data yet this round — fall back to unweighted mean so the
        // global model still moves rather than staying frozen at zero.
        for (const auto& p : clientParams)
            for (size_t i = 0; i < paramLen; ++i)
                avg[i] += p[i] / static_cast<double>(clientParams.size());
        return avg;
    }

    for (size_t c = 0; c < clientParams.size(); ++c) {
        double w = static_cast<double>(clientDataSizes[c]) / static_cast<double>(totalSamples);
        for (size_t i = 0; i < paramLen; ++i)
            avg[i] += clientParams[c][i] * w;
    }
    return avg;
}

// ---- FederatedCoordinator: drives synchronous multi-round FedAvg across all nodes ----
class FederatedCoordinator {
public:
    LocalModel globalModel;
    int roundNumber = 0;
    double lastGlobalLoss = 0.0;

    FederatedCoordinator() : globalModel(0.15) {}

    // Runs ONE full federated round:
    //   local training (epochs) at every node -> collect weights + sample counts
    //   -> weighted FedAvg -> update global model -> push global weights back down.
    // Returns the mean of the per-client local training losses (diagnostic only).
    double RunRound(std::vector<LocalModel*>& clientModels,
                     const std::vector<std::vector<FeatureLabelSample>*>& clientData,
                     int localEpochs) {
        std::vector<std::vector<double>> clientParams;
        std::vector<size_t> clientSizes;
        double sumLoss = 0.0;
        int trainedClients = 0;

        for (size_t c = 0; c < clientModels.size(); ++c) {
            std::vector<FeatureLabelSample>& data = *clientData[c];
            if (!data.empty()) {
                double loss = clientModels[c]->LocalTrainingLoop(data, localEpochs);
                sumLoss += loss;
                trainedClients++;
            }
            clientParams.push_back(clientModels[c]->GetParameters());
            clientSizes.push_back(data.size());
        }

        std::vector<double> aggregated = FedAvgAggregate(clientParams, clientSizes);
        if (!aggregated.empty()) {
            globalModel.SetParameters(aggregated);
        }

        // Broadcast the newly aggregated global weights back to every client so the
        // NEXT round's local training starts from the shared global model, as in
        // standard synchronous FedAvg.
        for (auto* m : clientModels) {
            m->SetParameters(globalModel.GetParameters());
        }

        roundNumber++;
        lastGlobalLoss = (trainedClients > 0) ? (sumLoss / trainedClients) : lastGlobalLoss;
        return lastGlobalLoss;
    }
};

// Global federated-learning coordinator instances (one per device population).
FederatedCoordinator* g_wipCoordinator = nullptr;
FederatedCoordinator* g_shsCoordinator = nullptr;

// ========== NodeMonitor with Real FL-IDS Integration ==========
// Each NodeMonitor now owns:
//   - a FederatedModel (domain risk engine: wASI / ISO-11073 derived metrics — unchanged)
//   - a LocalModel (the trainable classifier used in real federated learning)
//   - a rolling local training-set buffer of (feature, ground-truth-label) samples
struct NodeMonitor {
    ...
    std::vector<FeatureLabelSample> trainingBuffer;
    static constexpr size_t MAX_TRAINING_BUFFER = 500;
    double lastLocalMLProb = 0.0;   // most recent local-model prediction (0-1)
    double lastLocalMLLoss = 0.0;   // most recent local training-epoch loss

    NodeMonitor() {}

    NodeMonitor(const std::string& deviceType) : model(deviceType) {}

    LocalModel* GetLocalModel() { return &localModel; }
    std::vector<FeatureLabelSample>* GetTrainingBuffer() { return &trainingBuffer; }

    // Records one supervised training sample from a derived-metrics snapshot.
    // groundTruthAttack comes from the simulator's known attack state (mitmActive) —
    // in a real deployment this would instead be a confirmed/labelled incident.
    void RecordTrainingSample(const DerivedMetrics& dm, bool groundTruthAttack) {
        FeatureLabelSample s;
        s.features = { clamp0_100(dm.TRR) / 100.0,
                        clamp0_100(dm.PLR) / 100.0,
                        clamp0_100(dm.NDI) / 100.0,
                        clamp0_100(dm.JVI) / 100.0 };
        s.label = groundTruthAttack ? 1.0 : 0.0;

        trainingBuffer.push_back(s);
        if (trainingBuffer.size() > MAX_TRAINING_BUFFER)
            trainingBuffer.erase(trainingBuffer.begin(),
                                  trainingBuffer.begin() + (trainingBuffer.size() - MAX_TRAINING_BUFFER));
    }

    // Runs the trained local model forward-pass on the current derived metrics.
    double PredictML(const DerivedMetrics& dm) {
        std::array<double, 4> x = { clamp0_100(dm.TRR) / 100.0,
                                     clamp0_100(dm.PLR) / 100.0,
                                     clamp0_100(dm.NDI) / 100.0,
                                     clamp0_100(dm.JVI) / 100.0 };
        lastLocalMLProb = localModel.Forward(x);
        return lastLocalMLProb;
    }
    ...
    void ObserveFlowStats(const ns3::FlowMonitor::FlowStats& stats) {
        ...
        // ---- Scalar bookkeeping retained for CSV/legacy compatibility ----
        model.ObserveLoss(lossRate);

        // ---- Real local-model inference (trained via federated rounds elsewhere) ----
        double mlProb = PredictML(dm);

        // ---- Fused detection: ISO/IEEE 11073 wASI threshold OR local-model output ----
        bool wASICheck = (dm.ASI >= FLIDS_DETECTION_THRESHOLD);
        bool mlCheck    = (mlProb >= 0.5);
        int attack = (wASICheck || mlCheck) ? 1 : 0;
        model.lastAttackLabel = attack;
...
static const std::vector<std::pair<std::string, double>> kModelInfo = {
    {"Federated Local-Model IDS (real per-node SGD + multi-round FedAvg)", 0.0},
    {"Federated Local-Model IDS (real per-node SGD + multi-round FedAvg)", 0.0},
    {"Federated Local-Model IDS (real per-node SGD + multi-round FedAvg)", 0.0},
    {"Federated Local-Model IDS (real per-node SGD + multi-round FedAvg)", 0.0},
    {"Federated Local-Model IDS (real per-node SGD + multi-round FedAvg)", 0.0},
...
// ---- Runs one real federated round across the WIP sub-population (nodes 0-4) and
//      the SHS sub-population (nodes 5-8) separately, since each device type has its
//      own global model (their feature distributions differ, per journal Sec V-D). ----
void RunFederatedRound(std::vector<NodeMonitor>& nodeMonitors, uint32_t numWifiNodes) {
    std::vector<LocalModel*> wipModels, shsModels;
    std::vector<std::vector<FeatureLabelSample>*> wipData, shsData;

    for (uint32_t i = 0; i < numWifiNodes; ++i) {
        if (i <= 4) {
            wipModels.push_back(nodeMonitors[i].GetLocalModel());
            wipData.push_back(nodeMonitors[i].GetTrainingBuffer());
        } else {
            shsModels.push_back(nodeMonitors[i].GetLocalModel());
            shsData.push_back(nodeMonitors[i].GetTrainingBuffer());
        }
    }

    if (g_wipCoordinator && !wipModels.empty()) {
        double loss = g_wipCoordinator->RunRound(wipModels, wipData, kLocalEpochsPerRound);
        std::cout << "[FedAvg][WIP] Round " << g_wipCoordinator->roundNumber
                  << " complete | mean local training loss=" << std::fixed << std::setprecision(4)
                  << loss << " | global weights broadcast to " << wipModels.size() << " nodes" << std::endl;
    }
    if (g_shsCoordinator && !shsModels.empty()) {
        double loss = g_shsCoordinator->RunRound(shsModels, shsData, kLocalEpochsPerRound);
        std::cout << "[FedAvg][SHS] Round " << g_shsCoordinator->roundNumber
                  << " complete | mean local training loss=" << std::fixed << std::setprecision(4)
                  << loss << " | global weights broadcast to " << shsModels.size() << " nodes" << std::endl;
    }
}

// ========== Enhanced Flow Observation (now drives real per-sample FL training data) ==========
...
void EnhancedObserveFlowStats(NodeMonitor& monitor,
    ...
    // ---- Real FL: record a supervised training sample (features + ground-truth label) ----
    // Ground truth = current simulated attack state (mitmActive). A production deployment
    // would substitute confirmed/labelled security incidents for this signal.
    monitor.RecordTrainingSample(dm, mitmActive);

    // ---- Real FL: run the node's currently-trained local model on this flow ----
    double mlProb = monitor.PredictML(dm);
    ...
    if (g_blockchain) {
        int round = (deviceType == "WIP") ? (g_wipCoordinator ? g_wipCoordinator->roundNumber : 0)
                                           : (g_shsCoordinator ? g_shsCoordinator->roundNumber : 0);
        std::stringstream bd;
    ...
    if (g_assessmentLogger) {
        int round = (deviceType == "WIP") ? (g_wipCoordinator ? g_wipCoordinator->roundNumber : 0)
                                           : (g_shsCoordinator ? g_shsCoordinator->roundNumber : 0);
        double roundLoss = (deviceType == "WIP") ? (g_wipCoordinator ? g_wipCoordinator->lastGlobalLoss : 0.0)
                                                  : (g_shsCoordinator ? g_shsCoordinator->lastGlobalLoss : 0.0);
        g_assessmentLogger->LogAssessment(assessment, mlProb, roundLoss, round);
    }
    ...
    if (iso14971.actionLevel == "Critical" || iso14971.actionLevel == "Action") {
        std::cout << "\n=== EMERGENCY RESPONSE TRIGGERED ==="
                  ...
                  << "\n  Local FL model probability: " << mlProb
...
void DetectAndRecoverMITM(std::vector<NodeMonitor>& nodeMonitors, FederatedModel& /*globalModel*/) {
    ...
    for (uint32_t i = 0; i < nodeMonitors.size(); ++i) {
        bool scalarAnomaly = nodeMonitors[i].model.DetectAnomaly();
        bool mlAnomaly      = nodeMonitors[i].lastLocalMLProb >= 0.5;
        if (scalarAnomaly || mlAnomaly) {
            std::cout << "Node " << i << " detected anomaly (MITM) via "
                      << (mlAnomaly ? "trained local FL model" : "scalar loss check")
                      << " - triggering in-sim recovery" << std::endl;
...
void SchedulePeriodicDetection(double interval, double stopTime, Ptr<FlowMonitor> monitor, 
    ...
    // Per-node: compute wASI, record real FL training samples, run all standard assessments
    for (uint32_t i = 0; i < numWifiNodes; ++i) {
    ...
    // ---- REAL multi-round FedAvg: local training loop at every node this round,
    //      then weighted aggregation into the WIP / SHS global models, then broadcast
    //      the new global weights back down (journal-upgraded Section V-E). ----
    RunFederatedRound(nodeMonitors, numWifiNodes);
    ...
    // Print FL model status — REAL per-node local models + multi-round FedAvg
    std::cout << "[FL-IDS] Federated Learning Round Complete (Real Local-Model + FedAvg):" << std::endl;
    for (uint32_t i = 0; i < nodeMonitors.size(); ++i) {
        std::cout << " Node " << i << ": " << nodeMonitors[i].modelName
                  << " | trainingSamples=" << nodeMonitors[i].trainingBuffer.size()
                  << " | localMLProb=" << std::fixed << std::setprecision(4)
                  << nodeMonitors[i].lastLocalMLProb
                  << " | trainAccuracy=" << nodeMonitors[i].localModel.Accuracy(nodeMonitors[i].trainingBuffer)
                  << " | anomaly=" << (nodeMonitors[i].model.DetectAnomaly() ? "YES" : "no")
                  << std::endl;
    }
    std::cout << "WIP FedAvg round=" << (g_wipCoordinator ? g_wipCoordinator->roundNumber : 0)
              << " globalLoss=" << (g_wipCoordinator ? g_wipCoordinator->lastGlobalLoss : 0.0)
              << " | SHS FedAvg round=" << (g_shsCoordinator ? g_shsCoordinator->roundNumber : 0)
              << " globalLoss=" << (g_shsCoordinator ? g_shsCoordinator->lastGlobalLoss : 0.0)
              << std::endl;

    FederatedModel dummyGlobal; // legacy signature compatibility; recovery now checks real ML output
    DetectAndRecoverMITM(nodeMonitors, dummyGlobal);
...
int main(int argc, char *argv[]) {
    ...
    std::string flowOutputPath = "flowmonitor-stats_chapter6_integrated_shs.xml";
    ...
    std::cout << "\n=== IoMT FL-IDS & Blockchain Framework — Rajab et al. Journal ===\n"
              ...
              << "FL-IDS: REAL per-node local models (SGD) + multi-round weighted FedAvg,\n"
              << "        fused with wASI>=" << FLIDS_DETECTION_THRESHOLD << " domain threshold for detection.\n"
              << "================================================================\n\n";
    ...
    // Initialize the two real FedAvg coordinators (WIP population, SHS population)
    g_wipCoordinator = new FederatedCoordinator();
    g_shsCoordinator = new FederatedCoordinator();
    ...
    // Initialize NodeMonitors — each gets its own real LocalModel, trained via
    // synchronous multi-round FedAvg orchestrated by g_wipCoordinator / g_shsCoordinator
    std::vector<NodeMonitor> nodeMonitors;
    for (uint32_t i = 0; i < numWifiNodes; ++i) {
        std::string deviceType = (i <= 4) ? "WIP" : "SHS";
        nodeMonitors.emplace_back(deviceType);
        nodeMonitors[i].modelName    = kModelInfo[i % kModelInfo.size()].first;
        nodeMonitors[i].modelAccuracy = 0.0;  // empirically derived at runtime via LocalModel::Accuracy()

        // Seed every node's local model from its population's shared global model so
        // all clients start FedAvg from the same initial weights (standard FedAvg setup).
        if (deviceType == "WIP" && g_wipCoordinator)
            nodeMonitors[i].localModel.SetParameters(g_wipCoordinator->globalModel.GetParameters());
        else if (g_shsCoordinator)
            nodeMonitors[i].localModel.SetParameters(g_shsCoordinator->globalModel.GetParameters());

        std::cout << "[FL-IDS] Node " << i << " initialized: " << nodeMonitors[i].modelName
                  << " | DeviceType=" << deviceType
                  << " | Detection: wASI>=" << FLIDS_DETECTION_THRESHOLD
                      << " (ISO/IEEE 11073 MEDIUM) OR trained local-model prob>=0.5" << std::endl;
    }
    ...
    AnimationInterface anim("network-anim_chapter6_shs.xml");
    ...
    Simulator::Schedule(Seconds(simulationTime - 2.0), [&]() {
        ...
        // Final round: record real training samples from the last flow snapshot,
        // then run one more real local-training + FedAvg round before reporting.
        for (uint32_t i = 0; i < numWifiNodes; ++i) {
            std::string deviceType = (i <= 4) ? "WIP" : "SHS";
            for (const auto& s : stats) {
                EnhancedObserveFlowStats(nodeMonitors[i], s.second, s.first, deviceType);
        ...
        // Print FL-IDS final status — real per-node local models + multi-round FedAvg
        std::cout << "[FL-IDS] Final Federated Learning Status (Real Local-Model + FedAvg):" << std::endl;
        for (uint32_t i = 0; i < nodeMonitors.size(); ++i) {
            std::cout << " Node " << i << ": " << nodeMonitors[i].modelName
                      << " | trainingSamples=" << nodeMonitors[i].trainingBuffer.size()
                      << " | trainAccuracy=" << std::fixed << std::setprecision(4)
                      << nodeMonitors[i].localModel.Accuracy(nodeMonitors[i].trainingBuffer)
                      << " | localMLProb=" << nodeMonitors[i].lastLocalMLProb
                      << " | anomaly=" << (nodeMonitors[i].model.DetectAnomaly() ? "YES" : "no")
                      << std::endl;
        }
        std::cout << "WIP FedAvg total rounds=" << (g_wipCoordinator ? g_wipCoordinator->roundNumber : 0)
                  << " finalGlobalLoss=" << (g_wipCoordinator ? g_wipCoordinator->lastGlobalLoss : 0.0) << std::endl;
        std::cout << "SHS FedAvg total rounds=" << (g_shsCoordinator ? g_shsCoordinator->roundNumber : 0)
                  << " finalGlobalLoss=" << (g_shsCoordinator ? g_shsCoordinator->lastGlobalLoss : 0.0) << std::endl;
        std::cout << "Detection criterion: wASI >= " << FLIDS_DETECTION_THRESHOLD
                  << " (ISO/IEEE 11073 MEDIUM threshold) OR trained local-model prob >= 0.5" << std::endl;

        FederatedModel dummyGlobal;
        DetectAndRecoverMITM(nodeMonitors, dummyGlobal);
    });
...
Simulator::Run();
    ...
    if (g_assessmentLogger) g_assessmentLogger->GenerateSummaryReport();

    delete g_assessmentLogger;
    delete g_wipCoordinator;
    delete g_shsCoordinator;
