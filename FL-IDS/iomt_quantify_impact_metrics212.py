#!/usr/bin/env python3
"""
Merged IoMT MITM Full Pipeline
Combines comprehensive metrics calculation with full risk mapping and regulatory compliance
FIXED: NDI and JVI calculations now properly handle complete packet loss scenarios
ADDED: ISO11073/ISO14971 criticality mapping, PriorityScore, ActionLevel, and Excel metadata sheet
ADDED: Complete plotting functions for all metrics
"""
import os
import re
import glob
...
# Device weights (ISO 11073 inspired)
DEVICE_WEIGHTS = {
...
###############################################################################
# ISO/IEEE 11073 + ISO 14971 Clinical Criticality Mapping
###############################################################################
...
def compute_metrics_from_aggregates(agg_normal, agg_attack, device_type):
    ...
    FIXED: Properly handles complete packet loss scenarios for NDI and JVI calculations.
    """
    ...
    for fid in flow_ids:
        ...
        # Compute PriorityScore and ActionLevel
...
def build_word_report(derived_all):
    """Build comprehensive Word document with risk assessment."""
    ...
    doc.add_paragraph(f"This report includes FDA risk assessment, NHS risk register, CAF-DSPT mapping, and ISO 27001/27005 compliance.")
    ...
    if not topn.empty:
        ...
        hdr = t.rows[0].cells
        cols = ["Device", "FlowID", "ASI_score", "ASI_Severity", "RiskLabel", "NHS_RAG", "DominantMetric", "Mitigation"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="IoMT Comprehensive Analysis with Risk Mapping - FIXED NDI/JVI calculations and complete plotting")
